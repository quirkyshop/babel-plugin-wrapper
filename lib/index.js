...
module.exports = function (babel) {
  ...
  var importTraverseObject = {
    ...
    ImportDeclaration: function ImportDeclaration(path, data) {
      ...
      if (source.startsWith(wrapperPrefix)) {
        ...
        ['Button', 'Input', 'Modal'].forEach(function (localName) {
  ...
  function handleWrapper(file, wrapperData) {
    ...
    sortedKeys.forEach(function (wrapperKey) {
      ...
      if (params && Array.isArray(params)) {
        params.forEach(function (paramName) {
          objParams.push(t.objectProperty(t.identifier(paramName), t.identifier(libNameFormat(paramName))));
        });
      }
      ...
      if (wrapperKey.startsWith(repeaterPrefix)) {
        ...
        if (!hasDialog) {
          var ftDialogKey = wrapperKey.replace('repeater', 'dialog');
          var sourceKind = wrapperKey.split(repeaterPrefix)[1];
          dialogRefName = kindResultFormat('dialog', sourceKind);
          dialogRef.push(t.variableDeclaration('const', [t.variableDeclarator(t.identifier(dialogRefName), // _wrapper_result_antd
          t.callExpression(t.identifier(wrapperFormat(ftDialogKey)), // noform/lib/wrapper/antd
          [t.objectExpression(objParams)]))]));
        } else {
          dialogRefName = dialogName;
        }

        objParams = [].concat(objParams, t.objectProperty(t.identifier('Dialog'), t.identifier(dialogRefName)));
      }
