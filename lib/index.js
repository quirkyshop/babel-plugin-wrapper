  ...
  function handleWrapper(file, wrapperData, runtimeData) {
    ...
    if (weigthMap.dialog) {
      ...
      if (!weigthMap.wrapper) {
        ...
        if (weigthMap.repeater) {
          if (!runtimeData[_wrapperWeightKey]) {
            runtimeData[_wrapperWeightKey] = {};
          }

          if (_params2.indexOf('Input') === -1) {
            newParams.push('Input');
            runtimeData[_wrapperWeightKey]['Input'] = {};
          }

          if (hasSelectify && _params2.indexOf('Checkbox') === -1) {
            newParams.push('Checkbox');
            runtimeData[_wrapperWeightKey]['Checkbox'] = {};
          }

          if (hasSelectify && _params2.indexOf('Radio') === -1) {
