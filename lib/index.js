    ...
    if (weigthMap.dialog) {
      ...
      if (!weigthMap.wrapper) {
        wrapperData[_wrapperWeightKey] = {
          varName: _varName.replace('dialog', 'wrapper'),
          params: _params,
          refs: _params.map(function (refName) {
            return {
              varName: refName,
              source: libNameFormat(refName)
            };
          })
        };
      } else {
        var _params2 = wrapperData[_wrapperWeightKey].params;
        var newParams = [].concat(_params2);
        if (_params2.indexOf('Modal') === -1) newParams.push('Modal');

        if (weigthMap.repeater) {
          if (_params2.indexOf('Checkbox') === -1) newParams.push('Checkbox');
          if (_params2.indexOf('Radio') === -1) newParams.push('Radio');
        }
