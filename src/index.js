                  ...
                  if (t.isImportSpecifier(item)) {
                    ...
                    specificParams.push(
                        t.objectProperty(
                            ...
                            t.identifier(formatLibName),
                        )
                    );

                    specificRefs.push(t.variableDeclaration('const', [
                        t.variableDeclarator(
                            t.identifier(item.local.name),
                            t.memberExpression(
                                t.identifier(wrapperResult),
                                t.identifier(item.local.name)
                            )
                        )
                    ]));
            ...
            path.replaceWithMultiple([
                t.variableDeclaration('const', [
                    t.variableDeclarator(
                        t.identifier(wrapperResult),
                        t.callExpression(
                            t.identifier(wrapperFormat(source)),
                            [t.objectExpression(specificParams)]
                        )
                    )
                ])
            ].concat(specificRefs));
